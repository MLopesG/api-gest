...
func Categorias(c *fiber.Ctx) error {
	...
	if len(categorias) == 0 {
		return c.Status(404).JSON(fiber.Map{"status": false, "message": "Nenhum categoria cadastrada", "categorias": nil})
	}
...
func CadastrarCategoria(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(500).JSON(fiber.Map{"status": false, "message": "Não foi possivel cadastrar nova categoria", "error": err})
	}
...
func Categoria(c *fiber.Ctx) error {
	...
	if categoria.Id == 0 {
		return c.Status(404).JSON(fiber.Map{"status": false, "message": "Identificador da categoria não informado!", "categoria": nil})
	}
...
func DeletarCategoria(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(404).JSON(fiber.Map{"status": false, "message": "Falha ao deletar cadastro da categoria!", "usuario": nil})
	}
